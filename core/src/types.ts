...
export interface SSRHTMLElement extends Pick<HTMLElement, 'setAttribute' | 'removeAttribute' | 'addEventListener' | 'removeEventListener'> {
	/**
...
export type Directive<T = void, U extends SSRHTMLElement = HTMLElement> = (
	node: U,
	args: T,
) => void | {update?: (args: T) => void; destroy?: () => void};
export type SSRCompatibleDirective<T = void> = Directive<T, SSRHTMLElement>;

export interface SSRExecutableDirective<T = void, U extends SSRHTMLElement = HTMLElement> extends Directive<T, U> {
	/**
	 * The version of the directive that should be executed server-side.
	 */
	ssr: SSRCompatibleDirective<T>;
}
