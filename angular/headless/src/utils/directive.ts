...
import {isDirectiveExecutable} from '@agnos-ui/core/utils/directive';
...
export const useDirectiveForHost = <T>(directive?: AgnosUIDirective<T>, params?: T) => {
	...
	const callDirective = isPlatformServer(platform)
		? () => {
				if (directive && isDirectiveExecutable(directive)) {
					instance = directive(ref.nativeElement, params as T);
				}
			}
		: () => {
				if (plannedCallDirective || !directive || !isDirectiveExecutable(directive)) {
					return;
				}
